...
# from matplotlib import colorspip
...
class GridWorld:
    ...
    def __init__(
        ...
        self.valid_start_states = [state for state in self._init_states if not self._is_goal_state(self._state_list[state]) 
                            and not self._is_trap_state(self._state_list[state])
                            and not self._is_exit_state(self._state_list[state])]  # Assuming lava_state is stored in this manner
    ...
    def _get_next_state(self, state_coord: tuple, action: int) -> tuple:
        ...
        if action == 0:
            ...
            next_state_coord[1] += 1
        # print("1",state_coord,next_state_coord)
        ...
        # print("2",state_coord,next_state_coord)
    ...
    def step(self, action: int) -> tuple:
        ...
        # # Add other conditions for door, key, bait, etc.
            ...
            # done = True
            return 0, self._exit_reward, True, truncation
        ...
        # print("d",state_coord,next_state_coord,action)
        next_state = self._state_list.index(next_state_coord)
        
        # if self._step_count % 20 == 1:
        # print("f",self._current_state,next_state,action)
        if self._is_lava_state(next_state_coord):
            # Handle the lava state transition and assign reward
            # reward = self.step_reward # Assuming lava_reward is defined in your environment
            # done = True
            truncation = True
            return 0,self.step_reward, True, truncation
        ...
        # # Update the step count and check if the maximum number of steps has been reached
        if self._step_count >= self.max_step:
            # done = False
            truncation = True  # The episode is truncated because max steps were reached

        # # Additional checks for termination conditions
        # # Add your logic here if there are additional termination conditions
        if self._is_bait_state(next_state_coord):
            return next_state, self._bait_reward, False, truncation
        # Return the next state, reward, done flag, and info dictionary
    ...
    def reset(self) -> int:
        ...
        Returns:
            int: initial state
        """
        # Select a valid initial state that is not a door, key, bait, or lava state.
        # The initial state should be at the left of the lava and door, and the goal state will be at the right.
        # This is a simplification; you'll need to adjust it based on how you define your states.
        
        # print("valid",valid_start_states)
        # Assume we randomly select an initial state from the valid ones
        import random
        self._current_state = random.choice(self.valid_start_states)
        # Close the door if it's open
        if self._is_opened:
            self.close_door()  # You will need to implement this method if it doesn't exist
        # Reset the bait if it's bitten
        if self._is_baited:
            self.place_bait()  # You will need to implement this method if it doesn't exist

        # Return the initial observation and info dict
        # The observation could be the current state index or a more complex representation depending on your environment
        self._step_count = 0
        return self._current_state
